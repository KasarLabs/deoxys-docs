...
| clang             | Latest               | `apt-get install -y clang`                                       |
...
This command will start the Deoxys client with a basic set arguments which will begin the synchronization with Starknet mainnet automatically:
...
Navigate to the cloned Deoxys repository containing your `docker-compose.yml` file and run the following command to build and start the Deoxys client:
...
You can view the logs of the running Deoxys service using the following command:
...
```bash
curl -sSf https://raw.githubusercontent.com/KasarLabs/deoxys/main/scripts/launcher -o /tmp/launcher && chmod +x /tmp/launcher && /tmp/launcher
```
...
If you've decided to run your node in the background using `systemctl`, you may want to check your node logs using the following command:

```bash
sudo journalctl -u deoxys-node
```
...
<Callout type="warning" emoji="⚠">
  Parmis ces parametre il vous faudra obligatoirement un L1 endpoint access url
  que vous pourrez simplement obtenir en suivant le tutoriel present dans la
  categorie [L1 verification](../fundamentals/l1)
</Callout>
...
_Deoxys App is a project inspired by [Madara tsukuyomi](https://github.com/keep-starknet-strange/madara-tsukuyomi) from the Starknet exploration team._
