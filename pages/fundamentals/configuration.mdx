...
Here are the recomanded options for a quick and simple configuration of your Deoxys full node:

- **`--name <NAME>`**: The human-readable name for this node. It's used as network node name

- **`-d, --base-path <path>`**: Set the directory for Starknet data (dafault is `/tmp/deoxys`).
